<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.3/font/bootstrap-icons.css">
<style>
        body{
            background-image:linear-gradient(0deg, rgba(20, 19, 20, 0.461), rgba(3, 0, 2, 0.486)), url('/images/footer_fon.jpg');
            background-size: 100% auto;
            background-repeat: no-repeat;
            background-position: center center;
            height: 100%;
        }
        .b24-form-wrapper{
            margin: 0px auto;
            width: 100%;
            max-width: 448px;
            background: white;
            border-radius: 20px;
            box-shadow: rgb(60 66 87 / 12%) 0px 7px 14px 0px, rgb(0 0 0 / 12%) 0px 3px 6px 0px;
        }
        .b24-form-control-icon-after .b24-form-control{
            background-color: #fff!important;
        }
        .b24-form-btn{
           background-color: #5d39d8!important;
        }
        .b24-form-wrapper.b24-form-border-bottom{
            border-bottom: 0px solid #333!important;
        }
        .register{
            padding: 300px 0;
        }
        .b24-form-content{
            padding: 100px!important;
        }
        .b24-form-padding-side{
            padding-left: 60px!important;
            padding-right: 60px!important;
        }
        .b24-form-content .b24-form-padding-side{
            padding-left: 60px;
            padding-right: 60px;
            padding: 100px;
        }
        h1{
            text-align: center;
            color: #fff;
        }
        a{
            color: #fff;
            text-decoration: none;
        }
        a:hover{
            text-decoration: underline;
        }
</style>

<div class="register">
    <h1>
        <a href="{{ route('eng.index') }}"><i class="bi bi-arrow-left"></i> Home</a>
    </h1>
    <script data-b24-form="inline/107/cvf8d7" data-skip-moving="true">
        (function(w,d,u){
        var s=d.createElement('script');s.async=true;s.src=u+'?'+(Date.now()/180000|0);
        var h=d.getElementsByTagName('script')[0];h.parentNode.insertBefore(s,h);
        })(window,document,'https://office.astrum.uz/upload/crm/form/loader_107_cvf8d7.js');
    </script>
</div>
